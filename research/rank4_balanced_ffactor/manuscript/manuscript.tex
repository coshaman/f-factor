...
\author{Submission version -- author metadata to be supplied}
\date{}
...
\noindent
For the restricted problem, membership in NP is explicit: a certificate is
the 0/1 incidence vector of the selected edges, and all vertex degrees can be
checked in polynomial time. For unrestricted binary-encoded inputs, an
integer multiplicity vector is likewise polynomially encoded by its bounded
coordinates.
...
\begin{proof}[Proof of Theorem~\ref{thm:mengerian}]
...
The proof is independent of computation. Reproducibility details and exact
small-instance checks are supplied separately in the repository supplement;
they are falsification evidence, not a proof of any theorem.
